...
#### 4.1 概述
活动图是一种UML行为图，用于描述系统中一系列活动的执行流程。它关注的是"做什么"，展示了从一个活动到另一个活动的控制流。
...
##### 4.3.1 整体流程
```
开始 → 用户注册/登录 → 用户角色判断 → [学生流程/教师流程/管理员流程] → 结束
```

##### 4.3.2 学生流程
```
浏览课程 → 选课/退课 → 在线学习 → 查看作业 → 提交作业 → 查询成绩
```

##### 4.3.3 教师流程
```
创建/管理课程 → 上传课程内容 → 发布作业 → 批改作业 → 管理成绩
```
...
#### 4.4 特点与用途
- **关注流程**：强调"如何做"，展示完成特定任务所需的步骤
- **行为导向**：描述系统的动态行为和操作序列
- **用户视角**：从用户角度展示系统功能的使用流程
- **主要用途**：
  - 业务流程建模
  - 用例实现细节描述
  - 工作流展示
...
#### 5.1 概述
状态图是一种UML行为图，用于描述对象或系统在其生命周期内的状态变化。它关注的是"是什么状态"，展示了系统在不同时间点的状态以及状态之间的转换。

#### 5.2 符号说明
- **椭圆**：表示状态
- **箭头**：表示状态转换，箭头上的标签表示转换条件
- **起始点**：实心圆表示状态机的起始
- **结束点**：内部带实心圆的圆表示状态机的结束

#### 5.3 智慧学习平台状态图结构

##### 5.3.1 系统状态
```
系统离线 → 系统启动中 → 系统在线 → 系统维护中
```
...
##### 5.3.4 课程状态
```
课程草稿 → 课程待审核 → [课程已发布/课程已驳回] → 课程已归档
```

##### 5.3.5 作业状态
```
作业已创建 → 作业已发布 → 作业已提交 → 作业已评分
```

#### 5.4 特点与用途
- **关注状态**：强调"是什么"，展示系统在特定时间点的状态
- **状态转换**：描述触发状态变化的事件和条件
- **系统视角**：从系统角度展示其内部状态变化
- **主要用途**：
  - 对象生命周期建模
