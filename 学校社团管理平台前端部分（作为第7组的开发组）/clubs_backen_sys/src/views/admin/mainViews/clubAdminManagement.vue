  ...
  <div class="member-container">
    ...
    <el-card>
      ...
      <div class="search-filters">
        <!-- 搜索栏 -->
        <el-input v-model="searchQuery" placeholder="搜索社团管理员" prefix-icon="el-icon-search" class="search-input" />
        ...
        <el-button type="primary" @click="addStudent">添加社团管理员</el-button>
      </div>
      <el-table v-loading="loading" :data="filteredMemberList">
        <el-table-column prop="adminId" label="社团管理员id">
        ...
        <el-table-column prop="name" label="社团管理员">
          <template #default="scope">
        ...
        <el-table-column label="操作" fixed="right" min-width="140">
          <template #default="scope">
            <el-button type="success" @click="updateStudent(scope.row.admin_id)">更新资料</el-button>
            <el-button type="danger" @click="removeMember(scope.row.admin_id)">移除社团</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer v-model="showChangeRoleDialog" title="修改社团" size="40%" direction="rtl" :before-close="handleClose">
      <el-form ref="memberFormRef" :model="memberForm">
        <el-form-item label="社团管理员id" prop="adminId">
          <el-input v-model="memberForm.admin_id" placeholder="请输入管理员id" />
        </el-form-item>
        <el-form-item label="社团id" prop="clubId">
          <el-input v-model="memberForm.club_id" placeholder="请输入社团id" />
        </el-form-item>
        <el-form-item label="社团管理员" prop="clubName">
          <el-input v-model="memberForm.name" placeholder="请输入社团管理员" />
        </el-form-item>
        <el-form-item label="管理员账号" prop="username">
          <el-input v-model="memberForm.username" placeholder="请输入社团账号" />
        </el-form-item>
        <el-form-item label="管理员密码" prop="password">
          <el-input v-model="memberForm.password" placeholder="请输入社团密码" />
        </el-form-item>
    ...
    <el-drawer v-model="showAddStudentDialog" title="添加社团管理员" size="40%" direction="rtl" :before-close="handleClose">
      <el-form :model="addStudentForm">
        <el-form-item label="社团管理员id" prop="adminId">
          <el-input v-model="addStudentForm.admin_id" placeholder="请输入管理员id" />
        </el-form-item>
        <el-form-item label="社团id" prop="clubId">
          <el-input v-model="addStudentForm.club_id" placeholder="请输入社团id" />
        </el-form-item>
        <el-form-item label="社团管理员" prop="clubName">
          <el-input v-model="addStudentForm.name" placeholder="请输入社团管理员名" />
        </el-form-item>
        <el-form-item label="管理员账号" prop="username">
          <el-input v-model="addStudentForm.username" placeholder="请输入社团账号" />
        </el-form-item>
        <el-form-item label="管理员密码" prop="password">
          <el-input v-model="addStudentForm.password" placeholder="请输入社团密码" />
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
...
import type { ClubAdminItem, ClubAdminProfile } from "../../../types/member";
...
// 成员列表
const memberList = ref<ClubAdminItem[]>([]);

// 单个成员详情
const memberDetail = ref<ClubAdminItem | null>(null);
// 单个学生表单信息
const memberForm = ref<ClubAdminItem>({
  // 社团管理员id
  admin_id: "",
  // 社团id
  club_id: 1,
...
const addStudentForm = ref<ClubAdminProfile>({
  // 社团管理员id
  admin_id: "CA001",
  // 社团id
  club_id: 1,
...
const fetchMemberList = async () => {
  try {
    loading.value = true;
    const result = await adminMemberStore.getClubAdminList(
      currentPage.value,
      pageSize.value
    );
...
// 更新学生资料
const updateStudent = (id: string) => {
  console.log("改变角色", id);
  showChangeRoleDialog.value = true;
  memberDetail.value =
    memberList.value.find((member: ClubAdminItem) => member.admin_id == id) || null;
...
const confirmChangeRole = async () => {
  try {
    ...
    await adminMemberStore.updateClubAdmin(
      newForm
    );
    ElMessage.success(`修改${memberForm.value.admin_id}成功`);
...
const confirmAddStudent = async () => {
  try {
    ...
    ElMessage.success(`添加${addStudentForm.value.admin_id}成功`);
...
const removeMember = async (id: number) => {
  try {
    ...
    await adminMemberStore.deleteClubAdmin(id);
