...
本文档详细描述了学校社团管理系统的API接口，包括用户端(C端)、社团管理员后台和学校管理员后台的所有功能接口。系统支持学生、社团管理员和学校管理员三种角色的用户进行不同权限的操作。
...
系统采用JWT (JSON Web Token) 进行身份认证。登录成功后，服务器会返回一个JWT token，客户端需要在后续的请求头中携带此token：
...
**说明**: 学生只能看到自己已加入社团的活动，活动列表根据学生的token进行筛选。当一个学生没有参加任何社团时，他们查看活动列表将能看到所有社团的全部活动。
...
本文档详细描述了学校社团管理系统的API接口设计，涵盖了学生端、社团管理员后台和学校管理员后台的所有功能需求。API设计遵循RESTful规范，提供了完整的CRUD操作，支持权限控制和数据验证。系统采用JWT认证机制，确保API调用的安全性。

后续开发过程中，建议根据实际需求对API进行适当调整和扩展。